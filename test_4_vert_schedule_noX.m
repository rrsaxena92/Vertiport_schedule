...
% Defination of y^u_ij C9-C16
vertiOpt.Constraints.ytime1 = optimconstr(Nodes.all, flight_name_set, flight_name_set);
vertiOpt.Constraints.ytime2 = optimconstr(Nodes.all, flight_name_set, flight_name_set);
vertiOpt.Constraints.ytime3 = optimconstr(Nodes.all, flight_name_set, flight_name_set);
    ...
    for f1 = 1:length(flight_set)
        ...
        for f2 = 1:length(flight_set)
            ...
            if (f1 ~= f2) && common_node
                vertiOpt.Constraints.ytime1(u,i,j) = t_iu(j,u) >= t_iu(i,u) - (1-y_uij(u,i,j))*M;
                vertiOpt.Constraints.ytime2(u,i,j) = t_iu(i,u) >= t_iu(j,u) - (1-y_uij(u,a0,j))*M;
                vertiOpt.Constraints.ytime3(u,i,j) = t_iu(i,u) >= t_iu(j,u) - (1-y_uij(u,i,a0))*M;
            end
        end
    end
end
...
for n = 1:length(Nodes.all)
    ...
    for f1 = 1:length(flight_set)
        ...
        for f2 = 1:length(flight_set)
            ...
            if (f1 ~= f2) && common_node
                vertiOpt.Constraints.y2(u,i,j) = y_uij(u,j,i) >= y_uij(u,a0,j);
                vertiOpt.Constraints.y3(u,i,j) = y_uij(u,a0, j) + y_uij(u,i,j) <=  1;
                vertiOpt.Constraints.y4(u,i,j) = y_uij(u,j,i) >= y_uij(u,i,a0);
                vertiOpt.Constraints.y5(u,i,j) = y_uij(u,i,a0) + y_uij(u,i,j) <= 1;
            end
        end
    end
end
...
for f1 = 1:length(flight_set)
    for f2 = 1:length(flight_set)
        ...
        if (f1 ~= f2) && (r1 == r2)
            ...
            vertiOpt.Constraints.wake(i,j) = t_iu(j,r) >= t_iu(i,r) + Rsepij - (1-y_uij(r,i,j))*M;
        end
    end
end
...
for f1 = 1:length(dep_flight_set)
    for f2 = 1:length(dep_flight_set)
        ...
        if (f1 ~= f2) && (r1 == r2)
            ...
            vertiOpt.Constraints.TLOFenter2Dep(i,j) = t_iu(j,r) >= t_iu(i,ca) - (1-y_uij(r,i,j))*M;
        end
    end
end

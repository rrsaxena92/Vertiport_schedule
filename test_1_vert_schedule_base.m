...
m1=3*m;
FDT=(edge_length_before_TLOF/m1);
...
flight_set_struct = struct('name',[],'reqTime',[],'direction',[],'nodes',[],'edges',[],'TLOF',[],'taxi_speed',[], 'class', []);
...
flight_0 = struct('name',"0-0-0",'reqTime',0,'direction',[],'nodes',{Nodes.all},'edges',[],'TLOF',[],'taxi_speed',[],'class',[]);
a0 = flight_0.name;
...
vertiOpt.Objective = QtaxiTimeArr + Qtaxiout + QtaxiTimeDep + QtakeOff;
...
for f = 1:length(arr_flight_set)
    i = arr_flight_set(f).name;
    u1 = arr_flight_set(f).nodes(1);
    vertiOpt.Constraints.ARAPR_i(i) = t_iu(i,u1) == arr_flight_set(f).reqTime;
end
...
% Overtake C18
vertiOpt.Constraints.Overtake = optimconstr(string([Edges.taxi]), flight_name_set, flight_name_set);
for f1 = 1:length(flight_set)
    for f2 = 1:length(flight_set)
        i = flight_set(f1).name;
        j = flight_set(f2).name;
        if f1 ~= f2
            commonEdges =  setdiff(intersect(flight_set(f1).edges,flight_set(f2).edges,"stable"), [Edges.gate]);
...
% Collission C19
vertiOpt.Constraints.Collision = optimconstr(string([Edges.taxi]), flight_name_set, flight_name_set);
tmpEdges = string([Edges.taxi]);
for f1 = 1:length(flight_set)
    for f2 = 1:length(flight_set)
        i = flight_set(f1).name;
        j = flight_set(f2).name;
...
vertiOpt.Constraints.taxiSeparation1 = optimconstr(string([Edges.taxi]), flight_name_set, flight_name_set);
tmpEdges = cell([Edges.taxi]);
for f1 = 1:length(flight_set)
    for f2 = 1:length(flight_set)
        i = flight_set(f1).name;
        j = flight_set(f2).name;
...
if ~isempty(vertiOpt_sol.t_iu)
    ...
    flight_sol = validateOptSol_base(vertiOpt_sol, flight_set_0, inputs);
    endTime = datetime;
    fprintf(" End Time %s \n", endTime);
...
n=flight.nodes;
if(ismember(string(n(length(flight.nodes))),nodes.TLOF))
